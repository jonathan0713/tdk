...
void serialInit(){
    ...
    //local modes
    tty.c_lflag &= ~ICANON;     //diable canonical mode
    ...
    tty.c_iflag &= ~(IXON | IXOFF | IXANY);     //disable sofware flow control
    ...
    tty.c_cc[VTIME] = 10;       //wait 1 seconds for a read()
    tty.c_cc[VMIN] = 0;         //wait bytes for read() if VMIN is 0 it is not working
...
void CRC16Generate(serialData *msg){
    uint8_t CRCHigh = 0xff;     //high crc
    uint8_t CRCLow = 0xff;      //low crc  
    uint8_t index;

    for(int i = 0; i < (msg->length - 2); i++){
        index = CRCLow ^ (msg->data[i]);

        CRCLow = CRCHigh ^ CRCHighTable[index];
        CRCHigh = CRCLowTable[index];
    }
